...
def consume_item(item, t_inventory):
    if check_item(item, t_inventory):
        if t_inventory[item] <= 0:
            ...
            t_inventory[item] -= 1
            print(item+"을 사용하였습니다. 남은수량:"+str(t_inventory[item]))
    else:
...
def use_item():
    while True:
        ...
        if option == 0:
            ...
            what_item = input("아이템을 입력하세요.")
            consume_item(what_item,inventory)
...
character= {}
select_character = None # None=""=아무것도 없다 
#캐릭터 만들기 
...
while True:
    ...
    option = int(input("메뉴를 선택하세요.")) #int를 써서 정수문으로 바꿔줌
    if option == 0:
        print("종료합니다.")
        break
    elif option == 1:
        ...
        if check_character(temp_name, character):
            ...
            print(str(select_character)+"이(가) 선택되었습니다.")
        else:
            print(temp_name+"은존재하지않는 캐릭터입니다.")
    elif option == 4:
